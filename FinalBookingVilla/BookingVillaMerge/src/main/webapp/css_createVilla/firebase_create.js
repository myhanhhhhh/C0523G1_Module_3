...
function displayDownloadLink(url) {
    document.getElementById("display-map").style.backgroundImage = `url("${url}")`;
    document.getElementById("input-map").value = url;
}
function displayDownloadDetailLink(url){
    document.getElementById("input-detail").value = url;
}
function insertPicture(url){
    let data =  `<img src="${url}" class="array-detail-picture boxshadow-outset">`
    document.getElementById("control-array").innerHTML += data;
}
